...
public class EmailServiceImpl implements EmailService {
	...
	@Override
	public void email_send(String name, String email) { //텍스트 이메일 전송
		
		String pwCode = getCreateKey();
		System.out.println("랜덤 비밀번호 : " + pwCode);
		
		//문자만 전송
		SimpleMailMessage message = new SimpleMailMessage();
		
		message.setFrom("nijalmot@gmail.com"); //보낸 사람
		message.setTo(email); //보낼 사람
		message.setSubject(name + "님의 비밀번호가 초기화가 되었습니다.");
		message.setText(name + "님의 비밀번호가 초기화가 되었습니다.\n" + "[임시 비밀번호 : " + pwCode + " ]\n");
		mailSender.send(message);
		System.out.println("이메일이 발송되었습니다.");
	}
	
	@Override
	public void email_send2(String name, String email) { //html 이메일 전송
		...
		try {
			...
			message.setFrom(new InternetAddress("nijalmot@gmail.com"));//보내는 사람
			message.addRecipient(RecipientType.TO, new InternetAddress(email));//보낼 사람
